...
def create_embeddings_to_submit(data_dir,
    ...
    _ = get_embeddings_with_event_dataset(model, dataset_embeddings, batch_size=128, num_workers=2, embedding_path=output_dir / "embeddings.npy")
...
def main():
    ...
    parser.add_argument("--use_new_slope_columns", action='store_true', help="Use new slope columns for training and submission datasets")
    ...
    if args.use_new_slope_columns:
        row_to_tensor_train = partial(
            row_to_tensor,
            slope_columns=train_slope_columns_new,
        )
    else:
        row_to_tensor_train = partial(
            row_to_tensor,
            slope_columns=train_slope_columns_old,
        )
    ...
    if args.use_new_slope_columns:
        row_to_tensor_submission = partial(
            row_to_tensor,
            slope_columns=submission_slope_columns_new,
        )
    else:
        row_to_tensor_submission = partial(
            row_to_tensor,
            slope_columns=submission_slope_columns_old,
        )
